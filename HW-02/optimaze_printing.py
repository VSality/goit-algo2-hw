...
def optimize_printing(print_jobs: List[Dict], constraints: Dict) -> Dict:
    ...
    """
    # Перетворення словників у PrintJob
    jobs = [PrintJob(**job) for job in print_jobs]
    max_volume = constraints["max_volume"]
    max_items = constraints["max_items"]

    # Сортування завдань за пріоритетом (від найвищого до найнижчого) та часом друку
    jobs.sort(key=lambda x: (x.priority, x.print_time))

    print_order = []
    total_time = 0

    # Групування завдань для друку
    current_group = []
    current_volume = 0
